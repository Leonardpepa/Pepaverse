...
const configureIO = () => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("CREATE_FRIEND_REQUEST", (data) => {
      io.to((users[data.notification.receiver]).socketId).emit("FRIEND_REQUEST_NOTIFICATION", data);
    });

    socket.on("CREATE_LIKE_NOTIFICATION", (data) => {
      io.to((users[data.notification.receiver]).socketId).emit("LIKE_NOTIFICATION", data);
    });

    socket.on("CREATE_COMMENT_NOTIFICATION", (data) => {
      io.to((users[data.notification.receiver]).socketId).emit("COMMENT_NOTIFICATION", data);
    });
